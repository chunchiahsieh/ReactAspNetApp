...
import { List, ViewEdit, Delete } from "./components";
import { convertToSmOrgGroup4List } from "./components/List/ListModel";
import {
  convertToSmOrgGroup4EditView,
  initialColumns,
  convertEditColToUpdate,
  convertEditColToCreate,
} from "./components/ViewEdit/ViewEditModel";
...
export function SupplierAuthorize({
  ...
  const [listData, setListData] = useState(
    data?.map((d) => convertToSmOrgGroup4List(d, i18n.language as AvailableLang))
  );
  ...
  >("list");
  useEffect(() => {
    setListData(data?.map((d) => convertToSmOrgGroup4List(d, i18n.language as AvailableLang)));
  }, [data, i18n.language]);
      ...
      case "edit":
        if (_.keys(selected).length === 0) {
          ...
          const columns4show = !!data
            ? convertToSmOrgGroup4EditView(data[+_.keys(selected)[0]])
            : [];
          return (
            <ViewEdit
