import styles from './OC_BC_EditForm.module.scss';
import { useTranslation } from "react-i18next";
import { ESGForm } from "components";
import localeCommon from "styles/locales/common";
import dayjs from "dayjs";

export interface OC_BC_EditFormProps {
  title: string;
    columns: [];
    nColumn?: number;
}

export function OC_BC_EditForm({}: OC_BC_EditFormProps) {
  const [t, i18n] = useTranslation("common");
  return (
    <ESGForm
        title={t(localeCommon.Funcation_id.OC_BC_EditForm)}
        columns = {[
          ...
          { name: { en: t(localeCommon.OGHG_Organizational_Unit.street), zh: t(localeCommon.OGHG_Organizational_Unit.street) }
          , cType: "text" , value: ""},
