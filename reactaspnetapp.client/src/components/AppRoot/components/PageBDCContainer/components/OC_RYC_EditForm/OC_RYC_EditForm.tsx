import styles from './OC_RYC_EditForm.module.scss';
import { useTranslation } from "react-i18next";
import { ESGForm } from "components";
import localeCommon from "styles/locales/common";
import dayjs from "dayjs";

export interface OC_RYC_EditFormProps {
  title: string;
    columns: [];
    nColumn?: number;
}

export function OC_RYC_EditForm({}: OC_RYC_EditFormProps) {
  const [t, i18n] = useTranslation("common");
  return (
    <ESGForm
        title={t(localeCommon.Funcation_id.OC_RYC_EditForm)}
        columns = {[
