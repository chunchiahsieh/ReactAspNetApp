import styles from './OC_CHC_CreateForm.module.scss';
import { useTranslation } from "react-i18next";
import { ESGForm } from "components";
import localeCommon from "styles/locales/common";
import dayjs from "dayjs";

export interface OC_CHC_CreateFormProps {
  title: string;
  columns: [];
  nColumn?: number;
}

export function OC_CHC_CreateForm({

}: OC_CHC_CreateFormProps) {
  const [t, i18n] = useTranslation("common");
  return (
    <ESGForm
        title={t(localeCommon.Funcation_id.OC_CHC_CreateForm)}
        columns = {[
