import styles from './OC_RPC_EditForm.module.scss';
import { useTranslation } from "react-i18next";
import { ESGForm } from "components";
import localeCommon from "styles/locales/common";
import dayjs from "dayjs";

export interface OC_RPC_EditFormProps {
  title: string;
  columns: [];
  nColumn?: number;
}

export function OC_RPC_EditForm({}: OC_RPC_EditFormProps) {
  const [t, i18n] = useTranslation("common");
  return (
    <ESGForm
        title={t(localeCommon.Funcation_id.OC_RPC_EditForm)}
        columns = {[
